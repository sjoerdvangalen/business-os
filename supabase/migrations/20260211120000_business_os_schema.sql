...
SELECT
  ...
  cl.name as client,
  CASE
    WHEN ea.status = 'DISCONNECTED' THEN 'DISCONNECTED'
    WHEN ea.bounce_rate_3d > 5 THEN 'HIGH_BOUNCE'
    WHEN ea.overall_warmup_health < 80 THEN 'LOW_WARMUP'
    WHEN d.spf_valid = FALSE OR d.dkim_valid = FALSE THEN 'DNS_ISSUE'
    ELSE 'HEALTHY'
  END as health_status
FROM email_accounts ea
LEFT JOIN domains d ON d.id = ea.domain_id
