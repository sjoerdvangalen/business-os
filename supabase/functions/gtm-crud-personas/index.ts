...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method === 'GET' && !id) {
      ...
      let query = supabase.from('buyer_personas').select('*').order('created_at', { ascending: false })
    ...
    if (req.method === 'POST') {
      ...
      if (!body.persona_name) {
        return new Response(JSON.stringify({ error: 'Missing required field: persona_name' }), {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        })
      }
    ...
    console.error('gtm-crud-personas error:', (error as Error).message)
