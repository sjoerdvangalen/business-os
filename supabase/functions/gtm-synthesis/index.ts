...
serve(async (req) => {
  ...
  try {
    ...
    if (!chatResponse.ok) {
      ...
      return new Response(
        JSON.stringify({ success: false, error: `LLM API error: ${chatResponse.status}`, detail: errText.substring(0, 200) }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
    ...
    for (const key of REQUIRED_KEYS) {
      if (!synthesis[key]) {
        ...
        return new Response(
          JSON.stringify({ success: false, error: `synthesis missing key: ${key}`, raw: rawContent.substring(0, 500) }),
    ...
    if (strategyWriteError) {
      ...
      return new Response(
        JSON.stringify({ success: false, error: strategyWriteError.message }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }

    // Backwards-compat mirror: clients.gtm_synthesis (DEPRECATED_READONLY — do not use as primary source)
    ...
    if (mirrorError) {
      // Non-fatal: primary write succeeded, log mirror failure
      console.warn(`[${requestId}] Mirror to clients.gtm_synthesis failed (non-fatal):`, mirrorError.message)
    }

    // Fire-and-forget → gtm-doc-render (internal)
