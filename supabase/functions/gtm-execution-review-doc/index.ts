...
// ── Keyword profiling system prompt ─────────────────────────────────────────────
...
async function generateKeywordProfile(
  ...
  // Determine persona labels from campaign_matrix_seed filtered by icp_key
...
serve(async (req) => {
  ...
  try {
    ...
    if (strategyError || !strategyRow?.synthesis) {
      return new Response(
        JSON.stringify({ success: false, error: 'GTM synthesis not available' }),
    ...
    if (icpSegments.length === 0) {
      return new Response(
        JSON.stringify({ success: false, error: 'No ICP segments in synthesis' }),
        { status: 422, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }

    // Login to A-Leads (cookie-based)
    ...
    for (const segment of icpSegments) {
      ...
      // LLM call 1: industry keyword profile
      ...
      // LLM call 2: job title keyword profile
      ...
      const industryKeywordList = industryProfile.keywords
        .split(',').map(k => k.trim()).filter(Boolean)
      const jobTitleKeywordList = jobTitleProfile.keywords
        .split(',').map(k => k.trim()).filter(Boolean)
      const hqLocation = geoToHqLocation(geo)
      const companySizeCodes = employeeRangeToSizeCodes(employeeRange)

      // Squirrel company preview
      ...
      // Build keyword_profiles entry for this segment
    ...
    // Save keyword_profiles back to synthesis in gtm_strategies
    ...
    // Update client: stage, doc URL, workflow_metrics
