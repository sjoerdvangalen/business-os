 ...
 * Receives batches from GMaps scraper, creates companies + contacts
 *
 * Input: { scraper_job_id: string, companies: Array, client_id?: string }
 * Output: { processed: number, duplicates: number, companies_created: number }
 */
...
serve(async (req) => {
  ...
  try {
    const { scraper_job_id, companies, client_id } = await req.json();

    if (!scraper_job_id || !companies || !Array.isArray(companies)) {
      return new Response(
        JSON.stringify({ error: 'scraper_job_id and companies array are required' }),
