...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method === 'GET' && !id) {
      ...
      let query = supabase.from('icp_segments').select('*').order('created_at', { ascending: false })
    ...
    if (req.method === 'POST') {
      ...
      if (!body.segment_name) {
        return new Response(JSON.stringify({ error: 'Missing required field: segment_name' }), {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        })
      }
    ...
    console.error('gtm-crud-segments error:', (error as Error).message)
