...
serve(async (req) => {
  ...
  try {
    ...
    const { client_id, cell_id, sourcing_run_id: inputRunId, dry_run = false } = body
    ...
    if (!ALEADS_EMAIL || !ALEADS_PASSWORD) {
      return new Response(
        JSON.stringify({ success: false, error: 'ALEADS_EMAIL/ALEADS_PASSWORD not configured' }),
    ...
    if (cell_id) {
      ...
      if (cellRow?.icp_key) {
        ...
        if (targetSegments.length === 0) {
          return new Response(
            JSON.stringify({ success: false, error: `ICP segment ${cellRow.icp_key} not found in synthesis` }),
            { status: 422, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
          )
        }
      }
