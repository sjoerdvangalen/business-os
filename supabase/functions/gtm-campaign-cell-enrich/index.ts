...
import { PERSONA_REGISTRY } from '../_shared/persona_registry.ts'
import type { PersonaKey } from '../_shared/persona_registry.ts'
import type { VerticalKey } from '../_shared/vertical_registry.ts'
...
serve(async (req) => {
  ...
  try {
    ...
    if (messagingOutput.length === 0) {
      return new Response(
        JSON.stringify({ success: false, error: 'No messaging output found — run gtm-messaging-doc first' }),
