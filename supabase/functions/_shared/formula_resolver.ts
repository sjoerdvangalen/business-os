...
export function resolveFormula(input: ResolverInput): ResolverOutput {
  ...
  const persona = PERSONA_REGISTRY[persona_key]
  const vertical = VERTICAL_REGISTRY[vertical_key]

  if (!persona) throw new Error(`Unknown persona_key: ${persona_key}`)
  if (!vertical) throw new Error(`Unknown vertical_key: ${vertical_key}`)
  ...
  const overrides = synthesis_context?.persona_registry_overrides?.[persona_key]
  ...
  return {
    ...
    outcome_vocabulary: vertical.outcomes[persona_key] ?? [],
