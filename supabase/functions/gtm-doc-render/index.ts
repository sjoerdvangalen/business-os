...
function buildDemoTemplateA(): DocBlock[] {
  // Consulting Compact — numbered sections, label-value pairs, McKinsey-ish
  return [
    ...
    { style: 'NORMAL_TEXT', text: 'Klopt dit beeld? Wij horen graag of er aanpassingen nodig zijn op de doelgroepen of aanpak. Na uw akkoord starten we direct met de infrastructuur.' },
    { style: 'NORMAL_TEXT', text: '' },
...
function buildDemoTemplateB(): DocBlock[] {
  // Agency Story — narrative flow, proof-forward, more prose
  return [
...
function buildDemoTemplateC(): DocBlock[] {
  // Executive Brief — numbers first, 2-minute read, minimal
  return [
...
// GTM Strategy Blueprint — Internal Review (14 sections)
function buildInternalDoc(
  ...
  if (personaMap.length > 0) {
    ...
    // Fallback to buyer_personas for v1 synthesis
  ...
  // 12. Sourcing Feasibility Notes (placeholder until sourcing runs)
  lines.push(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`)
...
serve(async (req) => {
  ...
  try {
    ...
    const { client_id, mode } = body

    // ── Demo mode: create 3 template test docs ─────────────────────────────
    if (mode === 'demo') {
      const oauthCfg = Deno.env.get('GOOGLE_OAUTH_CONFIG')
      const svcAcct = Deno.env.get('GOOGLE_SERVICE_ACCOUNT_JSON')
    ...
    // Primary read source: gtm_strategies (canonical write truth)
    ...
    // Fallback: clients.gtm_synthesis (DEPRECATED_READONLY — backwards compat only)
