...
def score_record(r):
    ...
    for i, b in enumerate(bullets):
        ...
        if re.search(r"\d+\s+\b(seconds|minutes|hours|days|ms)\b", b, re.I):
    ...
    if persona == "cx" and len(bullets) >= 3:
        ...
        if ands != 1:
            ...
            score -= 1
        # B3 exactly one target and one constraint (allow one "and" for data_point)
        b3 = bullets[2]
        if b3.lower().count(" and ") + b3.lower().count(" or ") > 1:
...
for r in data:
    ...
    scores.append(s)
    if s < 8 or issues:
        print(f"--- {r['company']} | {r['persona']} | score {s} ---")
