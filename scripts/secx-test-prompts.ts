...
async function main() {
  ...
  console.log(`Cost per record:        $${(totalCost / testRecords.length).toFixed(5)}`);

  // Write results to file for analysis
  const dateStr = new Date().toISOString().split("T")[0];
  const outPath = join(import.meta.dir, "output", `secx-test-v2-${dateStr}.json`);
  const csvPath = join(import.meta.dir, "output", `secx-test-v2-${dateStr}.csv`);
  const fs = await import("fs");
  fs.mkdirSync(join(import.meta.dir, "output"), { recursive: true });
  fs.writeFileSync(outPath, JSON.stringify(results, null, 2));

  // Write CSV with bullets as markdown list in single cell
