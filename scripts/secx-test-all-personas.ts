...
const BASE_RECORDS: Omit<Record, "persona">[] = [
  {
    ...
    industry: "Financial", employees: "5001-10,000 employees",
  ...
  },
  {
    ...
    industry: "SaaS", employees: "5001-10,000 employees",
  ...
  },
  {
    ...
    industry: "Healthcare", employees: "5001-10,000 employees",
  ...
  },
  {
    ...
    industry: "Staffing", employees: "5001-10,000 employees",
  ...
  },
  {
    ...
    industry: "Manufacturing", employees: "1001-5000 employees",
...
async function main() {
  ...
  const results: { persona: Persona; rec: Record; bullets: string[] | null; usage: TokenUsage | null }[] = [];

  for (const persona of VALID_PERSONAS) {
    console.log(`\n${"=".repeat(60)}`);
    console.log(`PERSONA: ${persona.toUpperCase()}`);
    console.log("=".repeat(60));

    for (const base of BASE_RECORDS) {
      const rec: Record = { ...base, persona };
  ...
  for (const persona of VALID_PERSONAS) {
    ...
    const success = personaResults.filter(r => r.bullets !== null).length;
    console.log(`\n${persona.toUpperCase()}: ${success}/${personaResults.length} success`);
    for (const r of personaResults) {
      if (r.bullets) {
        const b1 = r.bullets[0];
        const wordCount = b1.split(/\s+/).length;
        console.log(`  ${r.rec.company}: B1=${wordCount}w | ${b1.slice(0, 70)}...`);
      }
    }
  }
  ...
  const outPath = join(import.meta.dir, "output", `secx-test-all-personas-${dateStr}.json`);
