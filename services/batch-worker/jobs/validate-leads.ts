 ...
 * Port van supabase/functions/validate-leads/index.ts naar Bun/Railway
 *
 * Input: { contact_ids?: string[], batch_size?: number, dry_run?: boolean,
 *          client_id?: string, cell_id?: string, sourcing_run_id?: string,
 *          method?: 'enrow' | 'omni' }
 * Output: { success: boolean, dry_run: boolean, method: string, stats: object }
 */
...
export interface ValidateLeadsResult {
  success: boolean;
  dry_run: boolean;
  method: string;
  stats: {
...
export async function validateLeads(opts: ValidateLeadsOptions): Promise<ValidateLeadsResult> {
  ...
  if (resolvedContactIds && resolvedContactIds.length > 0) {
    query = query.in('id', resolvedContactIds);
  } else if (cell_id) {
    return { success: true, dry_run, method, stats: { processed: 0, valid: 0, catch_all: 0, invalid: 0, unknown: 0, errors: 0, skipped_dnc: 0 } };
  }
  if (client_id) {
  ...
  if (method === 'omni') {
    ...
    if (!dry_run && updates.length > 0) {
      ...
      // Trigger Railway waterfall for invalid (not unknown — omni returns valid/invalid/catchall)
      ...
      for (const { contact } of invalidContacts) {
        if (RAILWAY_BATCH_WORKER_URL) {
          fetch(`${RAILWAY_BATCH_WORKER_URL}/waterfall/webhook`, {
            method: 'POST',
    ...
    for (const contact of contacts as Contact[]) {
      ...
      try {
        ...
        stats[enrowResult.status as keyof typeof stats]++;
        stats.processed++;
        ...
        if (!dry_run) {
          ...
          if ((enrowResult.status === 'invalid' || enrowResult.status === 'unknown') && RAILWAY_BATCH_WORKER_URL) {
            fetch(`${RAILWAY_BATCH_WORKER_URL}/waterfall/webhook`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ contact_id: contact.id, sourcing_run_id, force_search: true }),
  ...
  return { success: true, dry_run, method, stats };
}
