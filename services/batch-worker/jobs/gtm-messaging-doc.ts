...
interface MessagingDocOptions {
  client_id: string;
}

export async function runMessagingDoc(opts: MessagingDocOptions): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    if (cellsError) {
      return { success: false, error: `Failed to fetch cells: ${cellsError.message}`, request_id: requestId };
    }

    if (!cells || cells.length === 0) {
      return { success: false, error: 'No sourcing-approved cells found — ensure gtm-aleads-source ran and sourcing was approved', request_id: requestId };
    }
    ...
    if (!chatResponse.ok) {
      ...
      return { success: false, error: `LLM API error: ${chatResponse.status}`, request_id: requestId };
    }
    ...
    try {
      ...
      return { success: false, error: 'LLM output is not a valid JSON array', raw: rawContent.substring(0, 500), request_id: requestId };
    }
