...
interface CellSeedOptions {
  client_id: string;
}

export async function runCellSeed(opts: CellSeedOptions): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    // Fetch client_code for cell_code generation
    const { data: clientRow } = await supabase
      .from('clients')
      .select('client_code')
      .eq('id', client_id)
      .single();
    const clientCode = (clientRow?.client_code as string ?? 'UNK').toUpperCase();
    ...
    if (strategyError || !strategyRow) {
      return { success: false, error: `GTM strategy not found: ${strategyError?.message}`, request_id: requestId };
    }
    ...
    if (!synthesis) {
      return { success: false, error: 'Strategy synthesis not yet available', request_id: requestId };
    }
    ...
    if (validCells.length === 0) {
      return { success: false, error: 'No valid cells in campaign_matrix_seed', request_id: requestId };
    }
    ...
    for (const cell of validCells) {
      ...
      const geo = (cell.geo as string) ?? 'global';

      if (!solutionKey || !icpKey || !verticalKey || !personaKey) {
        errors.push(`Cell missing required keys: ${JSON.stringify(cell).substring(0, 100)}`);
        skipped++;
        continue;
      }

      // Generate cell_code from components if not present in seed
      // Format: CLIENT|LANG|solution-key|icp-key|vertical-key|persona-key|geo
      const lang = geo?.startsWith('NL') || geo?.startsWith('BE') ? 'NL' : 'EN';
      const cellCode = (cell.cell_code as string) ??
        `${clientCode}|${lang}|${solutionKey}|${icpKey}|${verticalKey}|${personaKey}|${geo}`;
