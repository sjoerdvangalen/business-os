...
BULLET 2 — Scale expert matching capacity across your [scope] [data_point] without [constraint].
  CRITICAL: This bullet MUST contain exactly ONE constraint. NEVER use "or" to add a second constraint.
  ...
  [data_point] (optional — insert ONLY if a third data point is available): Add after scope as "and [fact]". This must be a NUMBER or METRIC from the summary (e.g., "and $93B in assets", "and 12 markets"). NEVER use a second scope phrase here. NEVER invent.
  Constraint (pick exactly ONE): without adding any headcount
  WORD COUNT GUARANTEE: Base = "Scale" (1) + "expert matching capacity" (3) + "across your" (2) + scope (2-3) + "without" (1) + constraint (3) + "." = minimum 13 words. The constraint is always "without adding any headcount" (3 words). If data_point added, ensure total stays within 12-20.
