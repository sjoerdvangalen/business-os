...
BULLET 1 — Automate [routing] from [system] to [team+scale] [data_point] via API-first intent routing.
  ...
  [data_point] (optional — insert ONLY if a second data point is available from summary): Add after "counterparts" as "serving [fact]" or "across [fact]". Examples: "serving $93B in assets", "across 12 markets". NEVER invent.
  MUST include "via API-first intent routing"
  WORD COUNT GUARANTEE: Base = "Automate routing from" (3) + system (1-2) + "to" (1) + scale (1) + "technical counterparts" (2) + "via API-first intent routing." (4) = minimum 13 words. If data_point added, ensure total stays within 12-20.
